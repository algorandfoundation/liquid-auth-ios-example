...
struct ExampleLiquidAuthSigner: LiquidAuthChallengeSigner {
  ...
  func signLiquidAuthChallenge(_ challenge: Data) async throws -> Data {
    ...
    Logger.debug("Challenge as string: \(String(data: challenge, encoding: .utf8) ?? "nil")")
    Logger.debug("Challenge size: \(challenge.count) bytes")

    // Validate if we have JSON for validation
    if let challengeString = String(data: challenge, encoding: .utf8),
       let decodedJSON = Utility.decodeBase64UrlToJSON(challengeString),
       let schemaPath = Bundle.main.path(forResource: "auth.request", ofType: "json")
    {
      do {
        ...
        let valid = try ed25519Wallet.validateData(
          data: Data(decodedJSON.utf8),
          metadata: SignMetadata(encoding: Encoding.none, schema: schema)
        )
        guard valid else {
          throw NSError(domain: "com.liquidauth.error", code: -1,
                        userInfo: [NSLocalizedDescriptionKey: "Challenge validation failed"])
        }
        Logger.debug("✅ Challenge validation passed")
      } catch {
        Logger.warning("⚠️ Schema validation failed: \(error). Proceeding without validation.")
      }
    }

    do {
      let signature = try ed25519Wallet.rawSign(
